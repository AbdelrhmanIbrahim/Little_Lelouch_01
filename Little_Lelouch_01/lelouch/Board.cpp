...
void Board::IntializePiecesList()
{
...
long long Board::HAndVMoves(int s)
{
	s=63-Board120[s];
...
long long Board::KnightsMoves(int s)
{
...
long long Board::KingMoves(int s)
{
	int save =s;
	s=63-Board120[s];
...
bool Board::IsSquareAttacked(int AttackedSquare,int AttackingSide)
{
...
void Board::KingRookMovement(int piece,int from)
{
...
//making the move
void Board::MakingMove(Move& MyMove)
{
...
void Board::TakingMove(Move & MyMove,int side )
{
	int sq1=MyMove.From;
	int sq2=MyMove.To;
...
void Board::GenerateMoves(vector<Move>& MovesList,int WhichSide,int  _depth,bool OnlyCaptures)
{
	//All Pieces
...
void Board::GetMoveFromUser(string move)
{
...
void Board::PrintAllMoves(vector<Move>& AllGeneratedMoves)
{
	for(int x=0;x<AllGeneratedMoves.size();x++)
		PrintMove(AllGeneratedMoves[x]);

}
void Board::PrintPiecesList()
{
...
long long Board::AlphaBetaSearch(int depth,int alpha,int beta,int Player,int &StatrtingDepth,bool NullMove)
{	
	/*
...
//results table
void Board::TranspositionTable(int _score,int _depth,Move & _BestMove)
{
	if(BestMoveData.find(PositionHashKey)!=BestMoveData.end())
	{
...
void Board::IterativeDeepening(int depth,int alpha,int beta,int Player,int  TheDepth)
{
	//clear for search
	for(int x=0;x<NumberOfPossiblePieces;x++)
	{
...
long long Board::Quiescence(int alpha ,int beta,int side)
{
...
int Board::EvaluateThisPosition(int side)
{
	long long score=0;
	int Sign=1;
...
int Board::CenterControl(int side)
{
...
int Board::PawnsStructure(int side)
{
	int pen=0;
...
int Board::PopBit(unsigned long long MyBitBoard)
{
	int count=0;
	while(MyBitBoard!=0)
	{
...
bool Board::IsConnected(int & sq1,int & sq2)
{
...
bool Board::CheckConnection(int & sq1,int & sq2,bool rf)
{
...
int  Board::BadGoodBishop(int side)
{
...
int  Board::SlidingAttackPiecesOnTheKingFiles(int & file,unsigned long long &FILE,unsigned long long &OpenFile,int & side,int & Q,int & R,int & KingSafetyPen,int & AttackedFilePen)
{
...
void Board::MovesOrdering(vector<Move>& NonOrderdMoves,int Player)
{
